...
# DB (drizzle-kit) — package.json scripts로 정리 예정
...
- `optional(?)`은 *진짜로* 값이 없을 수 있는 곳에만 쓴다.
...
- ⚠️ `casing: 'snake_case'`는 **drizzle.config.ts와 런타임 `drizzle()` 호출 양쪽에 동일하게** 넣어야 일관된다. (설정 예정)
...
- 런타임 검증이 필요하면 `drizzle-zod` 또는 NestJS DTO + class-validator.
...
- 도구: **Supertest**(HTTP), 필요 시 **Testcontainers**(실제 DB 통합테스트), **MSW**(외부 API 모킹), **Playwright**(E2E 브라우저).
- 테스트 스위트가 5분을 넘기면 unit 비중을 늘리는 신호로 본다.
...
- 마이그레이션은 **점진적으로**. 한 번에 갈아엎지 않는다.

---

## 현재 진행 상황 (Drizzle 세팅)

- [x] 패키지 설치(`drizzle-orm`, `pg`, `dotenv`, `drizzle-kit`, `tsx`, `@types/pg`, `@nestjs/config`, `uuidv7`)
- [x] 예시 스키마 `src/database/schema/user.schema.ts`
- [ ] barrel `schema/index.ts`
- [ ] 주입 토큰/타입 `drizzle.constants.ts`
- [ ] `DrizzleModule` (전역, pg.Pool)
- [ ] `drizzle.config.ts` (+ `casing: 'snake_case'`)
- [ ] `AppModule`에 `ConfigModule` + `DrizzleModule` 연결
- [ ] `db:*` npm scripts
- [ ] 마이그레이션 생성/적용 검증
