...
- **FK 전파(cascade·set null)는 쓰기 어댑터를 우회한다 — 그래서 "부모 행 삭제"는 카운터를 조용히 깨뜨린다.** 판정은 삭제 대상마다 **"이 행을 지우면 비정규화 카운터가 드리프트하는가"** 하나다. 그런 테이블만 유스케이스가 명시적으로 지우고, 나머지는 DB cascade에 맡긴다 — 전부 앱으로 끌어오면 어댑터 여러 개를 주입하려고 모듈을 그만큼 import해야 하는데 사주는 게 없다(§0). 참조 구현: `DeleteUserUseCase`(`features/user/application/`)가 계정 삭제에서 `post_likes`**만** 지우고(→ 남의 글의 `posts.likeCount`) posts·sessions·user_plants·user_blocks는 cascade에 맡긴다. 전파 맵 전체는 `user.table.ts`의 테이블 doc이 단일 소스다.
  - **부모가 함께 사라지면 카운터 문제도 함께 사라진다** — 탈퇴자 자기 글의 좋아요·댓글은 `posts` cascade로 지워지는데, 카운터를 *든 행*(그 글)도 같이 없어지므로 드리프트가 성립하지 않는다. 명시 삭제가 필요한 건 **부모가 살아남는 경우**뿐이다(남의 글에 누른 좋아요).
  - **삭제 오케스트레이션도 테이블 소유권을 지킨다** — 자식 행을 지운 writer가 부모 카운터를 만지지 않는다. `PostLikeWriter.deleteAllByUser()`가 지운 행의 `postId` 집합을 **반환**하고, 그 값을 유스케이스가 `PostWriter.adjustLikeCounts(ids, -1)`에 넘긴다. 상관 서브쿼리(`UPDATE posts … WHERE id IN (SELECT … FROM post_likes …)`)로 합치면 왕복은 줄지만 ① `PostWriter`가 남의 테이블을 읽게 되고 ② "post_likes가 아직 안 지워졌음"이 **조용한 순서 불변식**이 된다(뒤집으면 카운터가 안 내려가는데 아무 신호가 없다). `returning`은 "지운 것"과 "감소시킬 것"을 같은 값으로 묶어 틀린 집합을 감소시킬 수 없게 만든다.
  - **때로는 삭제 정책 자체를 바꾸는 게 앱 보정보다 낫다.** `comments.authorId`를 cascade → set null로 바꾸자 "댓글이 지워져 `posts.commentCount`가 드리프트한다"는 문제가 **구조적으로 소멸**했다(행이 안 지워지므로). 보정 코드를 쓰기 전에 "그 행이 정말 지워져야 하나"를 먼저 묻는다.
  - **그 물음의 답은 "본인이 직접 지울 때 어떻게 되나"에서 나온다 — 새 정책을 발명하지 않는다.** 계정 삭제는 개별 삭제를 일괄 적용한 것이어야 한다("하나씩 다 지우고 탈퇴"와 "그냥 탈퇴"의 결과가 달라질 이유가 없다). 그래서 같은 users 삭제인데도 `posts`는 cascade이고 `comments`는 set null이다 — 글 삭제는 스레드 전체 소멸이고, 댓글 삭제는 답글이 있으면 자리를 지키는 삼분기이기 때문이다. **두 정책이 갈리는 건 절충이 아니라 각 테이블의 기존 규율을 그대로 따른 결과다.** (실제로 한때 둘 다 set null로 통일했다가 이 기준으로 뒤집었다 — post.table.ts authorId doc에 경위가 있다.)
...
- **왜 opt-in인가**: 전역 가드(secure-by-default)면 ① 공개마다 `@Public` opt-out + 보호마다 문서용 데코 = **이중 애노테이션**, ② 보호가 컨트롤러에 안 보이는 **암묵 동작**이 생긴다(명시 > 마법). 인증은 라우트마다 공개/보호/선택이 갈리는 **라우트 단위 결정**이라 opt-in.
  - ⚠️ **트레이드오프**: opt-in은 `@Authenticated` 누락 시 **fail-open**이다(전역 가드는 fail-closed) — 무표시가 "의도적 공개"와 "깜빡 누락" 둘 다를 뜻하므로 실수가 코드에 안 보인다.
  - **하지만 대부분의 누락은 조용하지 않다**: 보호 라우트는 거의 전부 `@CurrentUser()`를 소비한다 → 가드가 없으면 `req.user`가 `undefined`인데 타입은 `AuthUser`(non-nullable)라 `user.id`에서 TypeError(500). OpenAPI에서도 bearer 선언이 빠져 codegen 클라가 토큰을 안 보내므로 프론트 연동이 같이 깨진다. 즉 **열린 채로 조용히 도는 게 아니라 터진다**.
  - **진짜 위험한 부류 = "보호돼야 하는데 호출자 신원을 안 쓰는 라우트"** — 여기서만 누락이 정상 동작으로 보인다. 현재 유일한 사례는 `POST /files`(presign — 가드만 있고 user 미사용, 누락되면 누구나 S3 업로드 URL을 발급받는다). 이 부류는 **라우트 옆 401 E2E**로 막는다(`test/file.e2e-spec.ts`).
  - **allowlist 테스트("공개 목록에 없으면 인증 필수")는 도입하지 않는다** (2026-07 재검토, 이전 판은 "라우트가 늘면 도입"이라고 적혀 있었다): ① 위험은 라우트 *개수*가 아니라 위 부류의 개수에 비례하는데 현재 1개다, ② 공개 목록을 test 파일에 두는 건 이 절이 `@Public` opt-out을 거부한 이유(**이중 애노테이션**)를 라우트에서 *더 멀리 떨어진 곳*에서 되풀이하는 것이고, ③ 데코를 빠뜨린 사람의 최소 대응이 "allowlist에 한 줄 추가"라 검출력이 결국 리뷰 품질에 수렴한다. **재검토 트리거**: 신원을 안 쓰는 보호 라우트가 무리로 생길 때(관리자 API·배치 트리거·내부 호출) — 라우트 총개수는 트리거가 아니다.
