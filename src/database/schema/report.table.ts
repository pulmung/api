...
export const reports = pgTable(
  'reports',
  {
    ...
    //
    // **nullable인 이유**(회원탈퇴 도입 시 NOT NULL에서 풀었다): 남의 글에 단 댓글은
    // 작성자가 탈퇴해도 남으므로(comments.authorId set null) "살아있는 대상인데 작성자가
    // 없는" 상태가 정상이 됐다. NULL = 접수 시점에 대상 작성자가 이미 탈퇴.
    // 그 댓글도 신고할 수 있어야 한다 — **탈퇴가 신고 회피 수단이 되면 안 된다.**
    // (글은 작성자 탈퇴 시 함께 사라지므로 이 NULL이 안 생긴다 — 신고하면 그냥 404다.)
    // 기각안(NOT NULL 유지 → 탈퇴 유저의 댓글은 404로 신고 불가): 스레드에 남아 있는데
    // 신고 경로만 막히는 사각지대가 생긴다.
    // NULL 행은 누적 신호에서 빠지지만 그 유저가 존재하지 않으므로 잃는 신호가 0이다
    // (reporterId가 set null 후 유니크에서 빠지는 것과 같은 결 — 위 컬럼 doc).
    targetAuthorId: uuid(),
